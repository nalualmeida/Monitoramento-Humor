  ...
  try {
    const registros = await dbAll(
      ...
      if (registros.length > 0) {
        ...
        res.render("inicioDashboard", { user: user });;
      }
  } catch (err) {
...
app.get("/inicioDashboard", requireAuth, async (req, res) => {
  const userId = req.session.userId;
...
app.get("/estatisticas", requireAuth, async (req, res) => {
  ...
  try {
    const registros = await dbAll(
      "SELECT * FROM addHumor WHERE id_usuario = ?",
      [userId]
    );

      if (registros.length > 0) {
          res.render("estatisticas", { user: user, registros: registros });
      } else {
          res.render("inicioEstatisticas");
      }
  } catch (err) {
      console.error("Erro ao obter registros:", err.message);
      res.status(500).send("Erro ao carregar a página");
  ...
  try {
    ...
    const diasComRegistros = registros.map(registro => {
      const data = new Date(Date.UTC(
        parseInt(registro.data_atual.slice(0, 4)),
        parseInt(registro.data_atual.slice(5, 7)) - 1,
        parseInt(registro.data_atual.slice(8, 10))
      ));
      const dayDB = data.getUTCDate();
      return dayDB;
...
app.get("/inicioEstatisticas", requireAuth, (req, res) => {
  res.render("inicioEstatisticas.ejs");
});
